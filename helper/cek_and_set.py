from database.model import UserData, HookListService, DWResult
...
def cek_password(password: str, user):
    ...
    password_data = user.auth.password
...
async def cek_namaku_ada(nama:str):
    user = await UserData.get_or_none(nama=nama)
    if user is None:
        return False
    else:
        return True
...
async def set_name_unik(nama:str):
    while True:
        ...
        user = await UserData.get_or_none(nama=new_name)
        if user is None:
            return new_name
        
async def cek_data_user(namaORemail: str):
    nama = await UserData.get_or_none(nama=namaORemail)
    if nama:
        return nama
    
    else:
        email = await UserData.get_or_none(email=namaORemail)
        if email:
            return email
...
async def set_response_save_delusion(jumlah, data, first_id, user, input, ukuran):
...
async def cek_hookID(id: str):
    if id in config.hook_list.split(','):
        ...
        data = await HookListService.get_or_none(UUID=id)
        if data is None:
            return False